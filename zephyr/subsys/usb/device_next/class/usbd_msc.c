...
static void msc_queue_write(struct msc_bot_ctx *ctx)
{
	...
	while ((ctx->num_out_queued < MSC_NUM_BUFFERS) &&
		...
		if (ret) {
			...
			net_buf_unref(buf);
			msc_free_scsi_buf_ptr(ctx, scsi_buf);
...
static void msc_queue_bulk_in_ep(struct msc_bot_ctx *ctx, uint8_t *data, int len)
{
	...
	if (ret) {
		LOG_ERR("Failed to enqueue net_buf for 0x%02x", ep);
		msc_free_scsi_buf_ptr(ctx, data);
		net_buf_unref(buf);
		/* 6.6.2 Internal Device Error */
		msc_stall_and_wait_for_recovery(ctx);
	} else {
...
ep_request_error:
	...
	msc_free_scsi_buf(ctx, buf);
	if (buf->frags) {
		msc_free_scsi_buf(ctx, buf->frags);
	}
...
/* Replace line 873-874 with: */
#define DEFINE_SCSI_BUF(x, i)							\
	static uint8_t __attribute__((section(".sram1_data"))) __aligned(32)	\
		BUF_NAME(x, i)[CONFIG_USBD_MSC_SCSI_BUFFER_SIZE];
